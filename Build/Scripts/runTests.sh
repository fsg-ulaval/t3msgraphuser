...
read -r -d '' HELP <<EOF
t3msgraphuser test runner. Execute unit test suite and some other details.
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/t3msgraphuser/${1}"
else
    case ${TEST_SUITE} in
        acceptance)
            TEST_FILE="Web/typo3conf/ext/t3msgraphuser/Tests/Acceptance"
            ;;
        functional)
            TEST_FILE="Web/typo3conf/ext/t3msgraphuser/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/t3msgraphuser/Tests/Unit"
            ;;
    esac
fi
